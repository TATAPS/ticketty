...
async function getSingleContact(UUID) {
  try {
    const query = `SELECT BIN_TO_UUID(p.uuid, 1) as person_uuid, concat(p.given_name, " ", p.family_name) as contact_name, p.email, p.phone
    FROM company_contacts c INNER JOIN persons p
    ON c.person_uuid = p.uuid
    WHERE person_uuid = UUID_TO_BIN("${UUID}", 1);`;
